...
class GymPlans extends StatefulWidget {
  final UID;
  final user_type;
  final userImageUrl;
  final userDisplayName;

  const GymPlans({@required this.UID, @required this.user_type,@required this.userImageUrl,@required this.userDisplayName});


  @override
  _GymPlansState createState() => _GymPlansState();
}

class _GymPlansState extends State<GymPlans> {
          ...
          body: Container(
            ...
            child: Column(
              children: [
                ...
                FutureBuilder(
                  future: Provider.of<GymPlansProvider>(context,listen: false).fetchGymPlans(),
                  builder:(context, snapshot) {
                    if(snapshot.hasData){

                      var GymPlanData =[];
                      snapshot.data.docs.forEach((e){
                        if(e['CoachId'] == FirebaseAuth.instance.currentUser.uid) {
                          GymPlanData.add({
                            'CoachId': e['CoachId'],
                          ...
                          mainAxisSpacing: 20,
                            ...
                            children: GymPlanData.map((e) {
                              return InkWell(
                                ...
                                onTap: (){
                                    Navigator.of(context).push(MaterialPageRoute(builder: (context) => GymPlanDetails(GymPlanData: e,userId: e['CoachId'],userType: widget.user_type,),));
                                },
                                child: Container(
                                  decoration: BoxDecoration(
                                    borderRadius: BorderRadius.circular(20),
                                  ...
                                  child: SingleChildScrollView(
                                    child:  Column(
                                      ...
                                      children: [
                                        Align(alignment: Alignment.topRight,child: Icon(Icons.more_vert,color: Colors.white,),),
                                        CircleAvatar(radius: 50,backgroundImage: NetworkImage(e['CoachImageUrl']),),
                                        SizedBox(height: 10,),
                                        Text(e['GymName'],style: GoogleFonts.novaRound(fontSize: 20,color:  Color.fromRGBO(95, 0, 27, 1),),),
                                        Divider(color: Colors.grey,height: 10,),
                                        Row(
                                          mainAxisAlignment: MainAxisAlignment.center,
                                          children: [
                                            Text("Price ",style: TextStyle(color: Color.fromRGBO(0, 19, 94, 1),fontWeight: FontWeight.bold),),
                                            Text(e['Price']),
                                          ],
                                        ),
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            Text("Period ",style: TextStyle(color: Color.fromRGBO(0, 19, 94, 1),fontWeight: FontWeight.bold),),
                                            Text(e['MonthsOfPlan']),
                                          ],
                                        )
                                      ],
